...
public class EP_With_Preemption {
	...
	public static ObservableList<Process> EP_With_Preemption_alg( ArrayList<Process> L){
		i =0;
		...
		while(!P_Arrived.isEmpty() || !Tempp.isEmpty()) {
			...
			j=getHighestPriority(P_Arrived);
				...
				j=getHighestPriority(P_Arrived);  // change the working process if needed 
		...
		if(P_Arrived.isEmpty()!=true) {	
		 ...
		 if(P_Arrived.get(j).burst_time!=0 ) {
			...
			if(k!=j) { // don't increment the age if the process is working
			P_Arrived.get(k).Age++;	 
			}
	     ...
	     // set WTA and Wait time
	 /*    for(int in =0;in<L.size();in++)
	     {
	    	 L.get(in).setWTA((double)L.get(in).getTA()/(double)L.get(in).getBurst_time()); // WTA
	    	 L.get(in).setWait(L.get(in).getTA()-L.get(in).getBurst_time());
	    	 TOTALwaitTime+=L.get(in).getTA()-L.get(in).getBurst_time();
	     }*/
		...
		public static int getHighestPriority(ArrayList<Process> P_Arrived){ 
		    ...
		    int minValue = P_Arrived.get(0).priority;
		    int index = 0;
		    ...
		    for(int i=1;i<P_Arrived.size();i++){ 
		      ...
		      if(P_Arrived.get(i).priority < minValue){ // lower number = higher priority
		         minValue = P_Arrived.get(i).priority; 
		        index = i;
		      }
