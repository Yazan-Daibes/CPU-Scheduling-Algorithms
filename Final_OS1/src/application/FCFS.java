...
public class FCFS {
	...
	public static ObservableList<Process> FCFS_alg( ArrayList<Process> L){
		...
		public static int getArrival(ArrayList<Process> P_Arrived){ 
           if(P_Arrived.isEmpty()) {
        	   return -1;
           }
           else {
		    int minValue = P_Arrived.get(0).arrival_time;
		    int index = 0;
		      
		    for(int i=0;i<P_Arrived.size();i++){ 
		      if(P_Arrived.get(i).arrival_time < minValue){ 
		         minValue = P_Arrived.get(i).arrival_time; 
		        index = i;
		      } 
